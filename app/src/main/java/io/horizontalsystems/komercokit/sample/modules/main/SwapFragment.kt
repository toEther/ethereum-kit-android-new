...
class SwapFragment : Fragment() {
    ...
    private fun pathDescription(path: List<Token>): String {
        val parts = path.map { token ->
            if (token.isEther) "KMC" else (Configuration.erc20Tokens.firstOrNull { it.contractAddress == token.address }?.code
                    ?: token.address.hex)
        }
    ...
    private val fromTokenCode: String
        get() = viewModel.fromToken?.code ?: "KMC"

    private val toTokenCode: String
        get() = viewModel.toToken?.code ?: "KMC"
