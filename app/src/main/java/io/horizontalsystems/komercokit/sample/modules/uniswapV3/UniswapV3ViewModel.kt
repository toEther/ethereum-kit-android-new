package io.horizontalsystems.komercokit.sample.modules.uniswapV3
...
import io.horizontalsystems.komercokit.sample.modules.main.GasPriceHelper
...
class UniswapV3ViewModel(
    ...
    private val komercoAdapter: KomercoAdapter,
    private val gasPriceHelper: GasPriceHelper,
    private val signer: Signer
) : ViewModel() {

    private var uniswapV3Kit = UniswapV3Kit.getInstance(komercoKit)
    ...
    fun approve() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                val gasLimit = komercoKit.estimateGas(transactionData, gasPrice).await()
                Log.e("AAA", "gas limit: $gasLimit")
                val rawTransaction = komercoKit.rawTransaction(transactionData, gasPrice, gasLimit).await()
                val signature = signer.signature(rawTransaction)
                val fullTransaction = komercoKit.send(rawTransaction, signature).await()
...
//                Log.e("AAA", "swap SUCCESS, txHash=${fullTransaction.transaction.hash.toHexString()}")
    ...
    class Factory(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return UniswapV3ViewModel(komercoKit, erc20Adapter, komercoAdapter, gasPriceHelper, signer) as T
        }
    }
}
