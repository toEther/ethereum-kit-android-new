...
import io.horizontalsystems.komercokit.core.TransactionManager
...
class TransactionSyncManager(
    ...
    private val stateSubject = PublishSubject.create<KomercoKit.SyncState>()
    private val syncers = CopyOnWriteArrayList<ITransactionSyncer>()

    var syncState: KomercoKit.SyncState = KomercoKit.SyncState.NotSynced(KomercoKit.SyncError.NotStarted())
        private set(value) {
            field = value
            stateSubject.onNext(value)
        }
    val syncStateAsync: Flowable<KomercoKit.SyncState> = stateSubject.toFlowable(BackpressureStrategy.BUFFER)
    ...
    fun sync() {
        if (syncState is KomercoKit.SyncState.Syncing) return
        ...
        Single.zip(syncers.map {
            ...
            array.map { it as Pair<List<Transaction>, Boolean> }
                ...
                .subscribe({ transactions ->
                    ...
                    syncState = KomercoKit.SyncState.NotSynced(it)
