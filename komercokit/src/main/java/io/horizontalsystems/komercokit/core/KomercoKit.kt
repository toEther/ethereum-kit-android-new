...
import io.horizontalsystems.komercokit.api.core.*
import io.horizontalsystems.komercokit.api.jsonrpc.JsonRpc
import io.horizontalsystems.komercokit.api.jsonrpc.models.RpcBlock
import io.horizontalsystems.komercokit.api.jsonrpc.models.RpcTransaction
import io.horizontalsystems.komercokit.api.jsonrpc.models.RpcTransactionReceipt
import io.horizontalsystems.komercokit.api.models.AccountState
import io.horizontalsystems.komercokit.api.models.KomercoKitState
import io.horizontalsystems.komercokit.api.storage.ApiStorage
import io.horizontalsystems.komercokit.core.signer.Signer
import io.horizontalsystems.komercokit.core.storage.Eip20Storage
import io.horizontalsystems.komercokit.core.storage.TransactionStorage
...
import io.horizontalsystems.komercokit.crypto.InternalBouncyCastleProvider
import io.horizontalsystems.komercokit.decorations.DecorationManager
import io.horizontalsystems.komercokit.decorations.KomercoDecorator
...
import io.horizontalsystems.komercokit.transactionsyncers.KomercoTransactionSyncer
import io.horizontalsystems.komercokit.transactionsyncers.InternalTransactionSyncer
import io.horizontalsystems.komercokit.transactionsyncers.TransactionSyncManager
...
class KomercoKit(
    ...
    private val state: KomercoKitState = KomercoKitState()
) : IBlockchainListener {

    private val logger = Logger.getLogger("KomercoKit")
    ...
    companion object {
        ...
        fun getInstance(
            ...
            val address = komercoAddress(privateKey)
        ...
        fun getInstance(
            ...
            val apiDatabase = KomercoDatabaseManager.getKomercoApiDatabase(application, walletId, chain)
            val storage = ApiStorage(apiDatabase)

            val blockchain = RpcBlockchain.instance(address, storage, syncer, transactionBuilder)

            val transactionDatabase = KomercoDatabaseManager.getTransactionDatabase(application, walletId, chain)
            ...
            val erc20Database = KomercoDatabaseManager.getErc20Database(application, walletId, chain)
            val erc20Storage = Eip20Storage(erc20Database)

            val komercoTransactionSyncer = KomercoTransactionSyncer(transactionProvider, transactionSyncerStateStorage)
            ...
            transactionSyncManager.add(komercoTransactionSyncer)

            val komercoKit = KomercoKit(
                blockchain,
                transactionManager,
            ...
            blockchain.listener = komercoKit

            decorationManager.addTransactionDecorator(KomercoDecorator(address))

            return komercoKit
        }

        fun clear(context: Context, chain: Chain, walletId: String) {
            KomercoDatabaseManager.clear(context, chain, walletId)
        }
        ...
        private fun komercoAddress(privateKey: BigInteger): Address {
