...
class PeerGroupTest : Spek({
    ...
    describe("#syncState") {
        beforeEach {
            whenever(state.syncState).thenReturn(KomercoKit.SyncState.Syncing)
        }
    ...
    describe("#start") {
        ...
        it("sets syncing to state") {
            verify(state).syncState = KomercoKit.SyncState.Syncing
        }

        it("notifies listener that sync state changed to syncing") {
            verify(listener).onUpdate(KomercoKit.SyncState.Syncing)
        }
    ...
    describe("#didReceiveAccountState") {
        ...
        it("sets syncState to Synced") {
            ...
            verify(state).syncState = KomercoKit.SyncState.Synced
        }

        it("notifies listener that sync state changed to Synced") {
            peerGroup.didReceive(accountState, address, blockHeader)

            verify(listener).onUpdate(KomercoKit.SyncState.Synced)
        }
    }
    ...
    describe("#didAnounce") {
        ...
        context("when syncState is Synced") {
            beforeEach {
                whenever(state.syncState).thenReturn(KomercoKit.SyncState.Synced)
            }
