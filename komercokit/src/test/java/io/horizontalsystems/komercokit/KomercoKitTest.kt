package io.horizontalsystems.komercokit

/*
class KomercoKitTest {

    private val blockchain = mock(IBlockchain::class.java)
    private val addressValidator = mock(AddressValidator::class.java)
    private val state = mock(KomercoKitState::class.java)
    private val listener = mock(KomercoKit.Listener::class.java)
    private lateinit var kit: KomercoKit

    private val komercoAddress = "ether"

    private val transaction = KomercoTransaction().apply {
        hash = "hash"
        nonce = 123
        input = "input"
        from = "from"
    ...
    fun setUp() {
        ...
        whenever(blockchain.address).thenReturn(komercoAddress)
        kit = KomercoKit(blockchain, addressValidator, state)
    ...
    fun testReceiveAddress() {
        val komercoAddress = "eth_address"
        whenever(blockchain.address).thenReturn(komercoAddress)
        Assert.assertEquals(komercoAddress, kit.receiveAddress)
    }
    ...
    fun testRegister() {
        val address = "address"
        val balance = "123"
        val listenerMock = mock(KomercoKit.Listener::class.java)
    ...
    fun testTransactions() {
        val fromHash = "hash"
        val limit = 5
        val expectedResult = Single.just(listOf<KomercoTransaction>())
    ...
    fun testState_syncing() {
        whenever(blockchain.syncState).thenReturn(KomercoKit.SyncState.Syncing)
        val result = kit.syncState

        Assert.assertEquals(KomercoKit.SyncState.Syncing, result)
    }
    ...
    fun testState_null() {
        ...
        whenever(blockchain.getSyncStateErc20(address)).thenReturn(KomercoKit.SyncState.NotSynced)

        val result = kit.syncStateErc20(address)
        Assert.assertEquals(KomercoKit.SyncState.NotSynced, result)
    }
    ...
    fun testOnUpdateLastBlockHeight() {
        val height = 34L
        val erc20Listener = mock(KomercoKit.Listener::class.java)
    ...
    fun testOnUpdateState() {
        val syncState = KomercoKit.SyncState.Syncing
    ...
    fun testOnUpdateTransactions() {
        val transactions = listOf(KomercoTransaction())
